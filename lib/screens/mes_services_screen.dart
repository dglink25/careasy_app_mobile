...
class _MesServicesScreenState extends State<MesServicesScreen>
  ...
  // ── Domaines pré-chargés pour cette entreprise ────────────────────────
  ...
  void initState() {
    ...
    // Charger services ET domaines en parallèle dès l'ouverture
    _fetchServices();
    _fetchDomaines();
  }
  ...
  // ── Chargement des domaines de l'entreprise ───────────────────────────
  Future<void> _fetchDomaines() async {
    setState(() => _isLoadingDomaines = true);
    try {
      // 1. D'abord, tenter de lire les domaines déjà présents dans l'objet entreprise
      final rawDomaines = widget.entreprise['domaines'];
      if (rawDomaines is List && rawDomaines.isNotEmpty) {
        final parsed = _parseDomaines(rawDomaines);
        if (parsed.isNotEmpty) {
          if (mounted) setState(() { _domaines = parsed; _isLoadingDomaines = false; });
          return;
        }
      }

      // 2. Fallback : appel API /entreprises/{id}
  ...
  // ── Naviguer vers CreateServiceScreen avec domaines pré-chargés ───────
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => CreateServiceScreen(
          ...
          // On passe les domaines déjà chargés → zéro appel API supplémentaire
          preloadedDomaines: _domaines,
        ),
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        // ── Utilise la méthode dédiée qui passe les domaines pré-chargés ──
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Cette action va rendre votre service invisible aux clients.',
  ...
  Widget _priceBadge(String label, MaterialColor color, bool isSmallScreen) {
    return Container(
      ...
      decoration: BoxDecoration(
          color: color[50], borderRadius: BorderRadius.circular(8)),
      child: Text(label,
          style: TextStyle(
              fontSize: isSmallScreen ? 10 : 11,
              color: color[700],
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('Commencez par créer votre premier service.',
  ...
  Widget _buildShimmer(bool isSmallScreen) => ListView.builder(
        ...
        itemBuilder: (_, __) => Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                  color: Colors.grey[100]!, blurRadius: 8,
